...
class BaostockFinancialFetcher:
    ...
    def batch_fetch_with_retry(self, data_type, years=None, max_retries=5):
        """
        批量采集财务数据（带 Redis 断点重试）
        :param data_type: 数据类型
        :param years: 年份列表，None 表示当前年
        ...
        while retry_count <= max_retries:
            ...
            for i, stock_code in enumerate(stock_codes):
                try:
                    ...
                    for year in years:
                        if data_type == 'profit':
                            df = self.fetch_profit_data(stock_code, year)
                            if not df.empty:
                                self.mysql_manager.batch_insert_or_update('stock_profit_data', df, ['stock_code', 'statistic_date'])
                                rows_inserted += len(df)
                                self.update_record(stock_code, data_type, df['publish_date'].max())
                        
                        elif data_type == 'balance':
                            df = self.fetch_balance_data(stock_code, year)
                            if not df.empty:
                            ...
                            df = self.fetch_cash_flow_data(stock_code, year)
                            if not df.empty:
                            ...
                            df = self.fetch_growth_data(stock_code, year)
                            if not df.empty:
                            ...
                            df = self.fetch_operation_data(stock_code, year)
                            if not df.empty:
                            ...
                            df = self.fetch_dupont_data(stock_code, year)
                            if not df.empty:
                            ...
                            df = self.fetch_forecast_report(stock_code, year)
                            if not df.empty:
                                self.mysql_manager.batch_insert_or_update('stock_forecast_report', df, ['stock_code', 'publish_date'])
                                rows_inserted += len(df)
                                self.update_record(stock_code, data_type, df['publish_date'].max())
                        
                        elif data_type == 'dividend':
                            df = self.fetch_dividend_data(stock_code, year)
                            if not df.empty:
                                self.mysql_manager.batch_insert_or_update('stock_dividend_data', df, ['stock_code', 'announce_date'])
                                rows_inserted += len(df)
                                self.update_record(stock_code, data_type, df['announce_date'].max())
    ...
    def run_full_collection(self):
        ...
        for data_type, name in data_types:
            try:
                self.logger.info(f"\n[{data_type}] 采集{name}...")
                self.batch_fetch_with_retry(data_type, years=[self.current_year, self.current_year - 1], max_retries=3)
            except Exception as e:
